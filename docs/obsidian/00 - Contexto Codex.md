...
As Etapas 0A, 0B, 1A, 1B, 1C, 1D, 2A, 2B, 2C, 2D.1 e 2D.2A foram concluídas. A fundação em Next.js está publicada no repositório GitHub público `lucascout1010k/organizador-missoes`, com a branch principal `main`.
...
A Etapa 2D.2A evoluiu somente o contrato de Storage e a máquina de estados. `academic_materials.file_status` passou a admitir `validating`; em `pending_upload`, a policy de `SELECT` do objeto permite acesso apenas durante a operação interna `storage.object.upload`, enquanto a leitura normal continua negada. A leitura técnica do proprietário é admitida em `validating`, `ready` e `deleting`, mas a futura UI continuará abrindo ou baixando arquivos exclusivamente em `ready`. O runtime real aprovou 93/93 checks principais, 8/8 de isolamento A/B e 23/23 de cleanup, com negação anônima e zero erros. A arquitetura da aplicação não usará token de signed upload: o upload autenticado padrão Browser → Supabase Storage está planejado para a 2D.2B. As migrations 2D.1 permanecem imutáveis; frontend, upload/finalize e IA ainda não foram implementados.
